    ...
    const onFinish = async (values) => {
        if (fileList !== undefined){
            ...
            axios.post('api/rest/upload/objectCreate', fileData,{
                ...
                values.Logo = res.data.data.newName
                let result = await responseTamplase(`/api/rest/partners/objectUpdate/${el.id}`,'put',values)
                getData()
            }).catch(error=>{
                message.error(error.response.data.message)
            })
        }else {
            let res = await responseTamplase(`/api/rest/partners/objectUpdate/${el.id}`,'put',values)
            getData()
        }

    };
    let deleteItem = async (id)=>{
        let res = await responseTamplase(`/api/rest/partners/objectDelete/${id}`,'delete')
        getData()
    }
    useEffect(()=>{
        if (!!el.Logo ){
