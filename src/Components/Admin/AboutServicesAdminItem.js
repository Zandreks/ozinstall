    ...
    const onFinish = async (values) => {
        if (fileList !== undefined){
            ...
            axios.post('api/rest/upload/objectCreate', fileData,{
                ...
                let result = await responseTamplase(`/api/rest/aboutcolum/objectUpdate/${el.id}`,'put',values)
                getData()
            }).catch(error=>{
                message.error(error.response.data.message)
            })
        }else {
            let res = await responseTamplase(`/api/rest/aboutcolum/objectUpdate/${el.id}`,'put',values)
            getData()
        }

    };
    let deleteItem = async (id)=>{
        let res = await responseTamplase(`/api/rest/aboutcolum/objectDelete/${id}`,'delete')
        getData()
    }
    useEffect(()=>{
        if (!!el.Prev ){
